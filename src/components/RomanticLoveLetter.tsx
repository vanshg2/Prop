...
import { Heart, Sparkles, Flame, Eye, Lock, Feather, Stars, Music, Quote, ChevronRight, ChevronLeft, RefreshCw, Download, Check, Copy, Printer, Loader2, Mail, MailOpen } from "lucide-react";
...
export default function RomanticLoveLetter({ config }: RomanticLoveLetterProps) {
  ...
  // Unseal / Open Letter interaction
  ...
  return (
    <div className="w-full flex flex-col items-center select-none sm:select-text space-y-5 font-sans">
      ...
      <AnimatePresence>
        {petals.map((petal) => (
          <motion.div
            ...
            className="fixed pointer-events-none z-50 text-xl sm:text-2xl drop-shadow-md"
          >
            {petal.emoji}
          </motion.div>
        ))}
      ...
      {/* 1. CLOSED / SEALED STATE (Asks first to open the letter) */}
      {!isOpen ? (
        <motion.div
        ...
        >
          ...
          {/* Golden Wax Seal */}
          <motion.div
            animate={isUnsealing ? { scale: [1, 1.3, 0.8], rotate: [0, -15, 15, 0] } : { scale: [1, 1.05, 1] }}
            transition={{ duration: 2, repeat: isUnsealing ? 0 : Infinity, ease: "easeInOut" }}
            className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-gradient-to-tr from-rose-700 via-rose-600 to-red-500 text-amber-100 shadow-[0_6px_20px_rgba(225,29,72,0.4)] border-2 border-amber-300/80 flex items-center justify-center text-xl sm:text-2xl font-serif font-black mb-4 relative cursor-pointer"
            onClick={handleOpenLetter}
          >
            <span>V&R</span>
            <div className="absolute -inset-1 rounded-full border border-amber-300/40 animate-ping opacity-50 pointer-events-none" />
          </motion.div>

          <span className="text-xs font-black uppercase tracking-widest text-rose-600 mb-1">
            Private & Confidential 💌
          </span>

          <h3 className="font-cursive text-3xl sm:text-4xl lg:text-5xl font-extrabold text-[#881337] tracking-wide mb-2">
            Handwritten Love Letter from Vansh
          </h3>

          <p className="text-xs sm:text-sm text-gray-600 max-w-lg mb-6 leading-relaxed font-sans">
            A deeply intimate, unfiltered confession describing everything I feel about you—head to toe, nature, personality, and forever promises for <span className="font-bold text-rose-600">{config.partnerName || "Ridhima"}</span> 🌸🌹
          </p>
          ...
          <motion.button
            ...
            className="px-8 py-3.5 bg-gradient-to-r from-rose-500 via-pink-500 to-rose-600 hover:from-rose-600 hover:to-pink-600 text-white rounded-full font-bold text-sm sm:text-base shadow-lg shadow-rose-300/50 flex items-center gap-2 cursor-pointer transition-all disabled:opacity-75"
          >
            <MailOpen className="w-5 h-5 animate-bounce" />
            <span>{isUnsealing ? "Breaking Wax Seal... 🌸" : "Break Wax Seal & Unfold Letter 💌💋"}</span>
          </motion.button>
        </motion.div>
      ) : (
        /* 2. UNSEALED / OPENED STATE (Full Beloved Handwritten Letter) */
        <motion.div
        ...
        >
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 px-3.5 py-2 bg-rose-500/10 backdrop-blur-md rounded-2xl border border-rose-200/60 mb-3 shadow-2xs">
            ...
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              <button
                ...
                className="px-2.5 py-1 bg-rose-100 hover:bg-rose-200 text-rose-700 rounded-full text-[10px] font-bold border border-rose-300 transition-colors cursor-pointer font-sans"
          ...
          {/* THE HANDWRITTEN PARCHMENT PAPER */}
          <div
            ref={letterRef}
          ...
          >
            ...
            <div className="absolute top-4 right-4 sm:top-6 sm:right-6 flex flex-col items-center pointer-events-none select-none z-10">
              <div className="w-14 h-16 sm:w-16 sm:h-20 border-2 border-dashed border-rose-300/80 bg-rose-50/50 rounded-sm flex flex-col items-center justify-center p-1 shadow-2xs rotate-3">
                ...
                <span className="text-[7px] sm:text-[8px] font-black uppercase tracking-wider text-rose-500 mt-0.5 font-sans">
                  AIR MAIL
                </span>
              ...
              <div className="w-12 h-12 rounded-full border border-rose-400/40 -mt-5 -mr-4 flex items-center justify-center -rotate-12 pointer-events-none">
                <span className="text-[6px] font-black text-rose-400/80 uppercase tracking-widest text-center font-sans">
          ...
          <motion.div
            ...
            className="mt-4 bg-gradient-to-r from-rose-500/10 via-pink-500/15 to-purple-500/10 backdrop-blur-xl rounded-2xl p-4 sm:p-5 border border-rose-300/40 shadow-xs font-sans"
          >
          ...
          <motion.div
            ...
            className="mt-4 p-4 sm:p-5 bg-white/60 backdrop-blur-xl rounded-2xl border border-pink-200/80 shadow-xs flex flex-col sm:flex-row items-center justify-between gap-3 text-center sm:text-left font-sans"
          >
          ...
          <div className="flex items-center justify-center gap-2 mt-4 text-xs text-rose-600 font-bold font-sans">
